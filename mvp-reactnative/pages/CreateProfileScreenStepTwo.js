...
const CreateProfileScreenStepTwo = ({
  ...
  //   const response = await axios.get(`${hostname}/api/v1/user/getuserlist?limit=${limit}&skip=${skip}`);
  //   var c = 0;
  //   var v = 0;
  //   for (var i = 0; i < response.data.length; i++) {
  //     //if(profileState.accountType === createProfile.perso){
  //     if (profileState.firstName.length < 15) {
  //       //🇫🇷 Génère un nom d'utilisateur avec un suffixe numérique
  //       //🇬🇧 Generate a username with a numerical suffix
  //       var userNom = profileState.firstName + v;
  //       if (
  //         profileState.firstName === response.data[i].userName || userNom === response.data[i].userName
  //       ) {
  //         c++;
  //         v++;
  //         setNickName(profileState.firstName + c);
  //       }
  //       if (v === 0 && i === response.data.length - 1) {
  //         //🇫🇷 Si le nom d'utilisateur n'est pas déjà utilisé, il est attribué à l'utilisateur
  //         setNickName(profileState.firstName);
  //       }
  //     } else {
  //       //🇫🇷 Génère un nom d'utilisateur avec un suffixe numérique
  //       var userNomSup15 = profileState.firstName.substring(0, 13) + v;
  //       var userNomSup15v1 = profileState.firstName.substring(0, 14);
  //       if (
  ...
  //         c++;
  //         v++;
  //         setNickName(profileState.firstName.substring(0, 13) + c);
  //       }
  //       if (v === 0 && i === response.data.length - 1) {
  ...
  useEffect(() => {
    //🇫🇷 Vérifie si le prénom est défini, puis appelle la fonction VerifyUserName
    if (firstName) {
      VerifyUserName();
    }
  }, [firstName]);
  return (
    <ScrollView
      ...
      {errorMessageFirstName && (
        <Text style={styles.errorMessageTxt}>{errorMessageFirstName}</Text>
      )}
      ...
      {errorMessageLastName && (
        <Text style={styles.errorMessageTxt}>{errorMessageLastName}</Text>
      )}
      <View>
