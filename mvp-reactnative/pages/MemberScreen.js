...
import { Text, ScrollView, Image, StyleSheet, View, SafeAreaView, TouchableOpacity } from "react-native";
...
const MemberScreen = ({ route }) => {//(fr) fonction qui contient les infos du membre (gb) A function containing member information such as name, age, and points could be called a "Member Information"
  const member = route.params.member;
  const [user, setUser] = useState(null);
  const navigation = useNavigation();

  useEffect(() => {//(fr) fonction qui permet de récupérer les infos de l'utilisateur connecté (gb) A function that retrieves the information of the connected user
  ...
  const newChat = () => {//(fr) fonction qui permet de créer un nouveau chat (gb) A function that creates a new chat
    if (user) {
      addDoc(collection(database, "chats"), {
        ...
        Axios.post(
          `https://backoffice.socializus.com/api/private_chats`,//(fr) lien vers l'api (gb) link to the api
          { user1: user._id, user2: member._id, chat_id: newChatDoc.id },
          {
            headers: {//(fr) headers pour l'authentification (gb) headers for authentication
              authorization: `Bearer ${user.token}`
            }
          }
        )
          .then(() => {//(fr) redirection vers la page du chat (gb) redirection to the chat page
            navigation.navigate("PrivateChat", { chatId: newChatDoc.id });//(fr) redirection vers la page du chat (gb) redirection to the chat page
          })
          .catch((err) => console.log("MemberScreenError :", err)); 
      });
    }
  };
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        />
        <View style={styles.actionProfile}>{/**(fr) fonction qui contient les boutons "chat et block/ (gb)A function containing the "Chat" and "Block" buttons  */}
          <View style={styles.infoText}>{/**(fr)fonction qui contient les infos du membre (nom, age, points) (gb) A function containing member information such as name, age, and points could be called a "Member Information"  */}
            <Text style={styles.userPseudo}>{member.firstName}</Text>
            <Text style={styles.userAge}>{/** La fonction ager() permet de calculer l'age de la personne en fonction de sa date de naissance/ (gb) The "ager()" function would be called an "Age Calculator Function" in English. It calculates a person's age based on their date of birth  */}
              {ager(member.birthday)} {profile.years}
            </Text>
            <Text style={styles.userPoint}>435 {profile.points}</Text>
          </View>
          <View style={styles.btnContainer}>
            <TouchableOpacity style={styles.addFriend}>{/**(fr) fonction qui permet d'ajouter un ami (gb) A function that adds a friend  */}
          ...
          arg={[
            ...
            },
            {
              ...
              topNavArg: [
                ...
                {
                  link: "toast", to: () => <ActivitiesScreen fromStack="past activities" />
                },
                {
                  link: "tist", to: () => (<ActivitiesScreen fromStack="organized activities" />)
                }
              ]
            },
            {
              link: "Friends", to: () => <MembersScreen fromStack="male" />
            }
          ]}
        />
        <View style={styles.containerInfo}>
          ...
          <View style={styles.aboutMe}>
            ...
            <View style={styles.language}>
              ...
              <Text style={ styles.nativeLangTxt }>
                {profile.nativeLang}
              </Text>
              <View></View>
            </View>
