...
import styles from "./Styles/ActivitiesScreenCss.js"
...
const ActivitiesScreen = ({ fromStack, user, scr }) => {
  ...
  const VerificationData = async ()=>{
    try{
      ...
      //if(check === false){
      if(resultatUser.firstName === "null"  || resultatUser.lastName === "null" || resultatUser.userName === "null" || resultatUser.isPersonalAccount === "null" || resultatUser.city === "null"){ 
...
// console.log(groupedEvents);
  ...
  const renderItem = ({ item }) => {
    // const avatar = fetchAvatar(item.author);
      ...
      return (
        <>
          <ActivityCard_small event={item} connectedUser={connectedUser} scr={scr}/>
          {/* author={avatar}*/}
        </>
      );
      //}
  ...
  const renderSectionHeader = ({ section: { title } }) => {
    ...
    return (
      <View style={styles.header}>
        ...
        <ScheduleIcon style={ styles.scheduleIconStyle }
            width={24}
            height={24} />
    ...
    return (
      <>
        <EmailCheckDialog
         ...
         // scr={scr}
        />
        {splashLoad &&<View style={styles.splashContainer}>
          {/* <Image source={require('../assets/splash.svg')} style={styles.imgSplash} /> */}
          <SplashS style={styles.imgSplash} />
          <Spinner visible={loading} color="#000000" style={styles.spinnerLoad}/>
        </View>}
