...
// Endpoint that allow admin or organizer delete an part
router.post("/activities/removeUser", async (req, res) => {
...
router.post("/api/activities/forbiddenpeople", async (req, res) => {
  try {
    if (!req.headers.authorization) {
      ...
      res.status(402).json({ error: "Authorization header missing" }); // error message
      return;
    }

    const authorizationToken = req.headers.authorization.replace("Bearer ", ""); // token for authorization
    const user = await User.findOne({ token: authorizationToken }); // find user by token
    ...
    const o = req.body; // body of the request
    const activityId = o.activityId; // activity id
    const forbiddenList = o.forbiddenList; // list of people forbidden

    const activity = await Activity.findOne({ _id: activityId }); // find activity by search .
    if (!activity) {
      res.status(401).json({ error: "Incorrect activityId" });
      return;
    }
    ...
    //   }
    // }
    // j'ai changer le Boucle ici :La différence réside principalement dans la syntaxe et la lisibilité du code. La boucle for...of est généralement considérée comme plus concise et plus expressive, ce qui peut rendre le code plus clair et facile à comprendre.

    // add the activity in the list forbidden
    for (const element of forbiddenList) {
      if (activity.forbiddenPeople.indexOf(element) === -1) {
        activity.forbiddenPeople.push(element);
      }
    }
