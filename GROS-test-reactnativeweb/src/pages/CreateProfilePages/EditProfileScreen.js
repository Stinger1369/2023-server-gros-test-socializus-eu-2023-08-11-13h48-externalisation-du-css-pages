...
const EditProfileScreen = ({
  ...
  const HandleValidate = () => {
    ...
    /*🇬🇧 The en.json variable editProfile.errMess.t2022_ErrFirstName displays "Missing first name"*/
     setErrorMsgAvatarImage(editProfile.errMess.t2022_ErrAvatarImage); // Assurez-vous que ce message d'erreur est défini dans votre fichier JSON.
...
const allFieldsFilled = () => {
  return (
    avatarImage && firstName && nickName && city && language && appLanguage
  );
};
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={styles.savOrConButtons}>
          ...
          <TouchableOpacity
            style={[
              styles.bigBtn,
              !allFieldsFilled() ? styles.disabledButton : {}, // Appliquer un style grisé si les champs ne sont pas remplis
            ]}
            onPress={() => setStep(step + 1)}
            disabled={!allFieldsFilled()} // Désactiver le bouton si les champs ne sont pas remplis
          >
            <Text style={styles.btnText}>
