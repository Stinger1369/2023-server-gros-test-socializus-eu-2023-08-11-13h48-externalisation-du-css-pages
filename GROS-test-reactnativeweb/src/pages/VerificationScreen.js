...
const VerificationScreen = ({ setNumber, scr }) => {
  ...
  const requestPassword = async () => {
    ...
    try {
      const response = await axios.post(`${hostname}/api/v1/checkcode`, {
        email: params.email,
        code: code,
      });
      if (response.data.result === "OK") {
        //🇫🇷 Si la réponse est positive, on passe à la page de changement de mot de passe
  ...
  return (
    <ScrollView style={styles.forgotPage}>
      ...
      <View style={styles.btnContainer}>
        ...
        <TouchableOpacity
          onPress={() => {
            setNumber(2), navigation.navigate("Step1");
          }}
          style={styles.smallBtn}
        >
